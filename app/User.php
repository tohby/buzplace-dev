...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    public function getAvatarAttribute()
    {
        if ($this->attributes['avatar'] != null) {
            if (Str::of($this->attributes['avatar'])->startsWith('http')) {
    ...
    public function getDisplayNameAttribute()
    {
        if ($this->attributes['email_verified_at'] != null) {
            return $this->attributes['name'] . ' ' . '<svg width=".8em" height=".8em" viewBox="0 0 16 16" class="bi bi-patch-check-fll text-primary" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
